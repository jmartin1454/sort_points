...
print("Coil dimensions",a_out_z,a_out_y,a_in_y,a_out_x,a_in_x)

flux_ratio=a_in_x/(a_out_x-a_in_x)
print("Your flux ratio is %f"%flux_ratio)
...
for y in arange(side_spacing/2,a_out_y/2,side_spacing):
    n=n+1
    if(y<a_in_y/2):
        flux_ratio=body_spacing/side_spacing
        flux_ratio=flux_ratio+1
        print(flux_ratio)
        side_spacing_prime=body_spacing/flux_ratio
        if(mod(round((n-0.5)*2),round(flux_ratio))==0):
            n=n+1
        yprime=(n-0.5)*side_spacing_prime
        point1=(a_in_x/2,yprime,a_out_z/2)
        point2=(a_out_x/2,y,a_out_z/2)
        point3=(a_out_x/2,y,-a_out_z/2)
        point4=(a_in_x/2,yprime,-a_out_z/2)
    else:
...
ystart=body_spacing/2+(top_rung-1)*body_spacing
xstart=-a_in_x/(a_out_y-a_in_y)*(ystart-a_out_y/2)
top_spacing_mod=current/((a_out_y-a_in_y)/a_in_x+a_in_x/(a_out_y-a_in_y))
n=-trunc(xstart/top_spacing_mod)
print(xstart,ystart,top_spacing,top_spacing_mod,xstart/top_spacing_mod,n)
for x in arange(top_spacing/2,a_out_x/2,top_spacing):
    if(x<a_in_x/2):
        if(mod(n,turn_ratio_top)==0):
            n=n+1
        xprime=xstart+top_spacing_mod*n
        yposprime=a_in_y/2-(xprime-a_in_x/2)/(a_in_x/2)*(a_out_y/2-a_in_y/2)
        print(n,xprime,yposprime)
        n=n+1
    else:
